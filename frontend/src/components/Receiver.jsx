...
function Receiver({ userName, onLogout }) {
    ...
    const handleSearch = async (e) => {
        ...
        if (!searchData.area.trim()) {
            setMessage({ type: 'error', text: 'Please enter your area/locality' })
            return
        }
        ...
        try {
            ...
            if (result.success) {
                ...
                if (result.count === 0) {
                    setMessage({ type: 'info', text: result.message || 'No donors found' })
                } else {
                    setMessage({ type: 'success', text: result.message || `Found ${result.count} donor(s) in ${searchData.area}` })
                }
            } else {
    ...
    return (
        <div className="max-w-4xl mx-auto px-7 py-7 bg-gray-50 rounded-lg shadow-md">
            <div className="text-center mb-7">
                <h2 className="text-2xl font-bold text-red-600 m-0 mb-2">Find Blood Donors</h2>
                <p className="text-gray-600 m-0 text-sm">Welcome, {userName}! Find nearby blood donors in your area.</p>
            </div>
            ...
            <form onSubmit={handleSearch}>
                ...
                <div className="mb-5">
                    <label className="block mb-2 font-semibold text-gray-800">Your Area/Locality *</label>
                    <input
                        type="text"
                        name="area"
                        value={searchData.area}
                        onChange={handleInputChange}
                        placeholder="Enter your area (e.g., Gandipet, Nanakramguda)"
                        className="w-full px-3 py-3 border border-gray-300 rounded text-sm transition-all duration-300 focus:outline-none focus:border-red-600 focus:ring focus:ring-red-200"
                    />
                </div>
